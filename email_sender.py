...
def format_leads_html(leads):
    ...
    if not leads:
        html += """
            <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                <p>No new leads found this week. The scraper checked all sources — nothing new in Martin County. Will check again next week.</p>
            </div>
        """
    else:
        source_order = [
    ...
    html += """
            <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #ddd; font-size: 12px; color: #999;">
                <p><strong>Sources checked:</strong> FL DBPR licenses, Sunbiz LLC filings, Google News, HometownNewsTC, BusinessDebut.com</p>
                <p><strong>Tip:</strong> DBPR and Sunbiz leads are the hottest — these are businesses filing official paperwork, often before they even open.</p>
            </div>
        </div>
    </body>
    </html>
...
def send_email(leads, recipient_email=None):
    ...
    stakeholder_email = os.environ.get("STAKEHOLDER_EMAIL", "andrewholden@swipe.by")
    ...
    message = Mail(
        from_email=sender,
        to_emails=[recipient, stakeholder_email],
        subject=subject,
    ...
    try:
        ...
        print(f"  To: {recipient}, {stakeholder_email}")
